...
class JavascriptGenerator(CodeGenerator):
	...
	def visit_Class(self, class_node):
		...
		if class_node.super_classes:
			super_classes = class_node.super_classes.deepcopy()
		else:
	...
	def visit_Constructor(self, constructor):
		...
		if constructor.parent_class.statechart is not None:
			...
			for (i,node) in enumerate(constructor.parent_class.statechart.composites + constructor.parent_class.statechart.basics):
				self.fOut.write("this." + node.full_name + " = " + str(i) + ";")
		self.fOut.write()
		if constructor.parent_class.statechart is not None :
